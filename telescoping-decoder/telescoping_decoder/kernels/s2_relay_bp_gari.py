"""The S2 kernel for the GARI system: relay-BP with partial-row acceptance.

This module implements the same relay-memory and coset-quorum rules as
``s2_relay_bp.py`` and adds three inputs for the GARI decoding system:

  conv_rows    measure per-leg convergence over a subset of check rows (the
               relevant detector half) instead of all rows, via a dedicated
               residual kernel.
  weight_llr   an explicit weight vector for the min-weight selection among
               converged legs, so only the answer block is costed.
  layers       an explicit layer ordering, so the consistency-row blocks are
               processed before the detector rows.

With full-row convergence, channel weights, and contiguous layers, its
acceptance rules match the plain-system variant.
"""
...
# Non-templated kernels: residual (full + index-subset), per-shot weight
# reduction, select kernels.
...
// Residual over an explicit row-index subset (conv_rows). Identical
// math to residual_count, but block x indexes into conv_rows (length n_rel)
// instead of covering all m rows. Rows outside the subset never contribute
// to `residual`, so per-leg "converged" becomes the subset rule.
/** @brief Count violations only on selected GARI convergence rows.
 *  @param[in] posterior,c_neighbors_real,c_valid_real,synd Beliefs and checks.
 *  @param[in] conv_rows,B,n_rel,n,DC_REAL Selected rows and dimensions.
 *  @param[in,out] residual Per-shot count; must start at zero.
 *  @param[out] unsat Flags written at the original check-row indices.
 */
extern "C" __global__ void residual_count_idx(
    const float* __restrict__ posterior,         // (n, B)
    const int*   __restrict__ c_neighbors_real,  // (m, dc_max_real)
    const int*   __restrict__ c_valid_real,      // (m, dc_max_real)
    const unsigned char* __restrict__ synd,      // (m, B)
    const int*   __restrict__ conv_rows,         // (n_rel,)
    const int B, const int n_rel, const int n, const int DC_REAL,
    int* __restrict__ residual,                  // (B,)
    unsigned char* __restrict__ unsat            // (m, B); subset rows only
) {
    int k_row = blockIdx.x;
    int b = blockIdx.y * blockDim.x + threadIdx.x;
    if (b >= B || k_row >= n_rel) return;
    int c = conv_rows[k_row];
    const int* nbr = c_neighbors_real + (long long)c * DC_REAL;
    const int* val = c_valid_real + (long long)c * DC_REAL;
    int s = 0;
    for (int k = 0; k < DC_REAL; k++) {
...
/** @brief Compute each candidate's configured GARI correction cost.
 *  @param[in] posterior,weight_llr Beliefs and GARI variable costs.
...
extern "C" __global__ void compute_weight(
    const float* __restrict__ posterior,  // (n, B)
    const float* __restrict__ chan_w,      // (n,)  (weight vector, all >= 0)
...
/** @brief Apply one ordered GARI min-sum check update.
...
/** @brief Recompute posteriors for variables touched by a GARI layer.
...
# -----------------------------------------------------------------------------
# Helpers (plus explicit-layer support)
# -----------------------------------------------------------------------------
...
def _validate_layers(layers, m: int):
    """Explicit layers must exactly tile [0, m): every row in exactly one
    range. Processing ORDER is the list order (need not be ascending)."""
    norm = [(int(s), int(e)) for (s, e) in layers]
    for s, e in norm:
        if not (0 <= s < e <= m):
            raise ValueError(f"layer range ({s}, {e}) out of bounds for m={m}")
    by_start = sorted(norm)
    cursor = 0
    for s, e in by_start:
        if s != cursor:
            raise ValueError(
                f"explicit layers must tile [0, {m}) exactly; gap/overlap at "
                f"row {cursor} (next range starts at {s})")
        cursor = e
    if cursor != m:
        raise ValueError(
            f"explicit layers must tile [0, {m}) exactly; coverage ends at "
            f"{cursor}")
    return norm


def _build_layer_touched_vars_from_ranges(H: sp.spmatrix, layer_ranges):
    """Per-layer touched-variable subsets, in the given (processing) order."""
    H = sp.csr_matrix(H)
    K = len(layer_ranges)
    pieces = []
...
DEFAULT_S2_PHASE_GARI = S2RelayPhaseGari(
    "S2_gari", [(1.0, 0.0)],
...
class S2RelayBPGari:
    """Block-layered BP: IBM relay-BP + coset-quorum, plus generic
    optional kwargs — conv_rows (residual row subset), weight_llr (min-weight
    vector), layers (explicit ordered layer ranges). Standalone class."""

    BASE_SEED = 12345
    # Timing diagnostic: set False to skip the per-leg coset capture (the L@ê +
    # residual D2H that drives the quorum agreement check). Decode still freezes
    # at n_conv>=quorum, but no coset agreement is recorded (so conv_quorum is
    # all-False, so accuracy is not measured). Use only for pure-BP timing
    # throughput for a fair comparison vs decoders that don't track cosets.
    record_cosets = True
    ...
    def __init__(
        ...
        *,
        obs_matrix,
        K: int = 64,
        phase: S2RelayPhaseGari = DEFAULT_S2_PHASE_GARI,
        batch_size: int = 64,
        dc_pad: int = _DC_PAD,
        dv_pad: int = _DV_PAD,
        base_seed: int = 12345,
        ...
        # explicit ordered layers (default = contiguous K-blocking).
        if layers is not None:
            self._layers = _validate_layers(layers, self.m)
        else:
            self._layers = _layer_ranges(self.m, self.K)

        # residual row subset (default = all rows).
        if conv_rows is not None:
            cr = np.ascontiguousarray(np.asarray(conv_rows, dtype=np.int32))
            if cr.ndim != 1 or cr.size == 0:
                raise ValueError("conv_rows must be a non-empty 1-D index array")
            if int(cr.min()) < 0 or int(cr.max()) >= self.m:
                raise ValueError(
                    f"conv_rows indices out of range [0, {self.m})")
            self._conv_rows_d = cp.asarray(cr)
            self.n_conv_rows = int(cr.size)
        else:
            self._conv_rows_d = None
            self.n_conv_rows = self.m
        ...
        flat, offsets = _build_layer_touched_vars_from_ranges(H, self._layers)
        ...
        self._resid_idx = self._mod.get_function("residual_count_idx")
        ...
        self._channel_llr_d = cp.asarray(self.channel_llr)
        # min-weight selection vector (defaults to the channel llr).
        if weight_llr is not None:
            wl = np.ascontiguousarray(np.asarray(weight_llr, dtype=np.float32))
            if wl.shape != (self.n,):
                raise ValueError(
                    f"weight_llr must have shape ({self.n},); got {wl.shape}")
            self._weight_vec_d = cp.asarray(wl)
        else:
            self._weight_vec_d = self._channel_llr_d
        ...
        if c_degrees.size:
            print(
                ...
                f"conv_rows={self.n_conv_rows}/{self.m} "
                f"layers={len(self._layers)} "
                f"weight_vec={'custom' if weight_llr is not None else 'channel_llr'}"
            )
    ...
    def _build_bundle(self, B: int) -> _GraphBundle:
        ...
        def _capture_residual_weight():
            residual_d.fill(0)
            if self._conv_rows_d is not None:
                # residual over the conv_rows subset only — per-leg
                # "converged" is the subset (GARI relevant-half) rule.
                self._resid_idx(
                    (self.n_conv_rows, gy), (_BWARP,),
                    (
                        posterior_d, self._c_neighbors_d, self._c_valid_d,
                        synd_d, self._conv_rows_d,
                        np.int32(B), np.int32(self.n_conv_rows), np.int32(n),
            ...
            self._weight(
                (B,), w_block,
                (
                    posterior_d, self._weight_vec_d,
